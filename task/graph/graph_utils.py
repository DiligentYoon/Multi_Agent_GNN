...
#                  agent_id: int,
#                  inflation_radius_cells: int = 5) -> np.ndarray | None:
#     """
#     A* 알고리즘 기반 최적경로 탐색
#     """
...
#         start_min_r = max(0, start_pos[0] - inflation_radius_cells)
#         start_max_r = min(H-1, start_pos[0] + inflation_radius_cells)
#         start_min_c = max(0, start_pos[1] - inflation_radius_cells)
#         start_max_c = min(W-1, start_pos[1] + inflation_radius_cells)

#         valid = inflated_map[start_min_r:start_max_r, start_min_c:start_max_c] == map_mask["free"]
#         if np.any(valid):
#             start_pos = (np.where(valid)[0][0] + start_min_r, np.where(valid)[1][0] + start_min_c) 
#         else:
...
#         end_min_r = max(0, end_pos[0] - inflation_radius_cells)
#         end_max_r = min(H-1, end_pos[0] + inflation_radius_cells)
#         end_min_c = max(0, end_pos[1] - inflation_radius_cells)
#         end_max_c = min(W-1, end_pos[1] + inflation_radius_cells)

#         valid = inflated_map[end_min_r:end_max_r, end_min_c:end_max_c] == map_mask["free"]
#         if np.any(valid):
#             end_pos = (np.where(valid)[0][0] + end_min_r, np.where(valid)[1][0] + end_min_c) 
#         else:
